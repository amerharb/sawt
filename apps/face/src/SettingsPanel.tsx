 ...
 * game. Which faces are on the board is a question about what you see, so that
...
type Props = {
	...
	emotions: { code: string, emoji: string }[],
...
export default function SettingsPanel({ settings, languages, emotions, caching, cachedCount, locked, t, uiLanguage, uiLanguages, onSetUiLanguage, onChange, onSetSort, onClearCache, shareUrl }: Readonly<Props>) {
	...
	const toggleFace = (code: string) => {
		const hiddenFaces = settings.hiddenFaces.includes(code)
			? settings.hiddenFaces.filter(c => c !== code)
			: [...settings.hiddenFaces, code]
		onChange({ ...settings, hiddenFaces })
	}
	...
	const showAllFaces = () => onChange({ ...settings, hiddenFaces: [] })
	const hideAllFaces = () => onChange({ ...settings, hiddenFaces: emotions.map(e => e.code) })
				...
				<div className="settings-panel" role="dialog" aria-label={t('settings.title')}>
					...
					{tab === 'see' && (
						<div className="settings-tabpanel" role="tabpanel" id="settings-panel-see" aria-labelledby="settings-tab-see">
							...
							{/* which faces are on the board is a question about what you see */}

							<div className="settings-row">
								<div className="settings-select-all">
									<button
										type="button"
										aria-label={t('selectAllFaces')}
										title={t('selectAll')}
										disabled={locked}
									...
									<button
										type="button"
										aria-label={t('deselectAllFaces')}
										title={t('deselectAll')}
										disabled={locked}
										onClick={hideAllFaces}
									>
									⬜
									</button>
								</div>
								<div className="settings-face-grid" role="group" aria-label={t('group.faces')}>
									{emotions.map(e => {
										const shown = !settings.hiddenFaces.includes(e.code)
										return (
											<button
												key={`setting-face-${e.code}`}
												type="button"
												className={shown ? 'face-toggle' : 'face-toggle hidden'}
												aria-pressed={shown}
												aria-label={e.code}
												title={e.code}
												disabled={locked}
												onClick={() => toggleFace(e.code)}
											>
												{e.emoji}
											</button>
										)
