...
// how the verbs are ordered on the main screen:
//   'code'   — by the verb's English slug (the default)
//   'name'   — by the verb's name, in whichever language the app speaks it —
...
export type Settings = {
	...
	// localized languages, independent of the content (verb-name) language
	uiLanguage: UiLanguage,
	// codes the user chose to hide from the main screen; empty = show everything,
	// so newly added languages/verbs are visible by default
	hiddenLanguages: Language[],
	hiddenVerbs: string[],
	// when on, all visible sounds are downloaded to the cache, and newly shown
	// languages/verbs are cached as soon as they are enabled
	flightMode: boolean,
	// order the verbs are shown in on the main screen
	sortMode: SortMode,
	// the frozen random order (verb codes) used when sortMode === 'random'.
	// covers every verb, including hidden ones, so a verb keeps its slot when shown.
	randomOrder: string[],
}
